...
import symonenko.controller.AbstractController;
import symonenko.entitymanager.EntityManager;
import symonenko.model.*;

import symonenko.service.AbstractService;
import symonenko.service.impl.*;
...
public class ViewOperations<T, K extends Serializable> {
        ...
        while (true) {
            ...
            try {
                if (fieldType == Integer.class) {
                    ...
                    Float value = getValidatedFloat(inputText);
                    if (value == null) continue;
                    field.set(entity, value);
                ...
                } else if (fieldType == City.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<City, Integer> CityService = new CityServiceImpl();
                    field.set(entity, CityService.findById(id));
                } else if (fieldType == Country.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<Country, Integer> clusterProgramService = new CountryServiceImpl();
                    field.set(entity, clusterProgramService.findById(id));
                } else if (fieldType == CurrentInfo.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<CurrentInfo, Integer> speakerService = new CurrentInfoServiceImpl();
                    field.set(entity, speakerService.findById(id));
                } else if (fieldType == Owner.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<Owner, Integer> studentGroupService = new OwnerServiceImpl();
                    field.set(entity, studentGroupService.findById(id));
                } else if (fieldType == OwnerHasStantion.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<OwnerHasStantion, Integer> studentService = new OwnerHasStantionServiceImpl();
                    field.set(entity, studentService.findById(id));
                } else if (fieldType == Sale.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<Sale, Integer> lecturerService = new SaleServiceImpl();
                    field.set(entity, lecturerService.findById(id));
                } else if (fieldType == SolarPanel.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<SolarPanel, Integer> subjectService = new SolarPanelServiceImpl();
                    field.set(entity, subjectService.findById(id));
                } else if (fieldType == Station.class) {
                    Integer id = getValidatedInteger(inputText);
                    if (id == null) continue;
                    AbstractService<Station, Integer> subjectService = new StationServiceImpl();
                    field.set(entity, subjectService.findById(id));
                }else {
    ...
    private void getUserHintForField(String fieldName, Class<?> fieldType) {
        String hint = null;
        if (fieldType == Integer.class || fieldType == City.class || fieldType == Country.class
                || fieldType == Owner.class || fieldType == OwnerHasStantion.class || fieldType == CurrentInfo.class
                || fieldType == Sale.class|| fieldType == SolarPanel.class || fieldType == Station.class) {
    ...
    private Float getValidatedFloat(String inputText) {
        Float value;
        try {
